...
R_USER_LIB = os.path.expanduser('~/R/library')
os.makedirs(R_USER_LIB, exist_ok=True)


# Force rpy2/R to prefer the user library and avoid checking the empty system site-library
os.environ['R_LIBS_USER'] = R_USER_LIB
# Point R_LIBS_SITE at the user lib (or unset) to prevent the noisy message about /usr/lib/R/site-library
os.environ['R_LIBS_SITE'] = R_USER_LIB
# Optionally remove any R_PROFILE that may trigger package checks at startup
os.environ.pop('R_PROFILE', None)
os.environ.pop('R_PROFILE_USER', None)
...
try:
    ...
    try:
        robjects.r('.libPaths(c("{}", .libPaths()))'.format(R_USER_LIB))
    except Exception:
        pass
...
# Invoke Threads
    ...
    try:
        ...
        logger.info(f"AFTER THREADS: successful={successful_regions}, failed={failed_regions}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run analysis tasks (simple CLI)")
    parser.add_argument("--gwas", required=True, help="Path to GWAS file")
    parser.add_argument("--plink", required=True, help="PLINK prefix/dir") 
    parser.add_argument("--chrom", required=False, type=int, help="chromosome (optional)")
    ...
    try:
        # Run munge preprocessing
        munged_df, munged_path = munge_sumstats_preprocessing(
            args.gwas,
            args.out,
            ref_genome="GRCh37"
        )
        
        # Filter significant variants - Unpack both values
        significant_df, sig_path = filter_significant_variants(
            munged_df,
            args.out,
            p_threshold=5e-8
        )
        
        # Run COJO analysis - Pass only the DataFrame
        cojo_results = run_cojo_per_chromosome(
            significant_df,  # Pass only the DataFrame, not the tuple
            args.plink,
            args.out,
            maf_threshold=0.01
        )
        ...
        # Rest of the code...
        if isinstance(cojo_results, tuple) and len(cojo_results) == 2:
            cojo_df, cojo_path = cojo_results
        else:
            cojo_df = cojo_results

        # Create region batches and run fine-mapping

        region_batches = create_region_batches(cojo_df)
        
        temp_sumstats_path = save_sumstats_for_workers(munged_df)
        logger.info(f"[MAIN] Temporary sumstats file for workers: {temp_sumstats_path}")
        finemap_params = {
            "seed": 42,
            "window": 2000,
            "L": 5, 
        ...
        with ProcessPoolExecutor(max_workers=3) as executor:
            future_to_batch = {
                executor.submit(
                    ...
                    (batch, batch_id, temp_sumstats_path, additional_params)
                ): batch_id 
                for batch_id, batch in enumerate(region_batches)
            }
        ...
        logger.info(f"Analysis completed. Results saved to {args.out}")
